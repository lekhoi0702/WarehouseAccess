...
const formState = reactive({ deptCode: '', deptName: '' });
...
async function onSubmit() {
  ...
  const code = formState.deptCode.trim();
  const name = formState.deptName.trim();
  
  if (!code || !name) {
    masterDataErrorMessage.value = 'Dept Code and Dept Name are required.';
    return;
  }

  const payload = { deptCode: code, deptName: name };

  try {
    const res = await saveDepartment(payload, formMode.value === 'edit');
    if (res?.success) {
      showToast(formMode.value === 'edit' ? 'Department updated' : 'Department created');
      resetForm();
    } else {
      masterDataErrorMessage.value = res?.message || 'Save department failed';
    }
  } catch (e) {
...
function editItem(item) {
  ...
  formState.deptName = item.deptName || '';
}

async function deleteItem(item) {
  if (!confirm(`Are you sure you want to delete department ${item.deptName}?`)) return;
  try {
    const res = await removeDepartment(item.deptCode);
    if (res?.success) {
...
<template>
  <div class="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-6 text-left">
    ...
    <div class="bg-slate-50 border border-slate-200/60 rounded-2xl p-5 space-y-4 h-fit">
      <h3 class="text-sm font-extrabold text-slate-800 m-0">{{ formMode === 'create' ? 'Add Department' : 'Edit Department' }}</h3>
      
      <div class="space-y-3">
        <div class="flex flex-col gap-1 text-left">
          <label class="text-xs font-bold text-slate-500">Dept Code</label>
          <input 
            type="text" 
            v-model="formState.deptCode" 
            :disabled="formMode === 'edit'"
            class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs outline-none focus:border-primary/50 disabled:opacity-5"
          />
        </div>

        <div class="flex flex-col gap-1 text-left">
          <label class="text-xs font-bold text-slate-500">Dept Name</label>
          <input 
            type="text" 
        ...
        <table class="w-full text-left border-collapse text-xs">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-if="departmentItems.length === 0">
              <td colspan="4" class="p-8 text-center text-slate-400 font-semibold">No departments found.</td>
            </tr>
            <tr v-else v-for="item in departmentItems" :key="item.deptCode" class="hover:bg-slate-50/40 transition">
              <td class="p-4"><span class="font-mono font-bold text-slate-600 bg-slate-100 px-2 py-0.5 rounded">{{ item.deptCode }}</span></td>
              <td class="p-4"><strong class="text-slate-800 text-sm">{{ item.deptName }}</strong></td>
